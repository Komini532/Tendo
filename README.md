...
フィールド倍率が 0.7〜3.5 という扱える範囲に収まる。

### 階層と滞在時間

通常進行は 7 階層。敵Lv は 1 撃破 +1 で、自Lv も 1:1 で追従するよう `exp` を選んでいるので、
**その階層に居る撃破数 = 上限 − 参入Lv** になる。

| 階層 | フィールド | 参入Lv | 上限 | 滞在撃破数 | 目安 |
|---|---|---|---|---|---|
| 0 | 草原 | 1 | 100 | 99 | 25 分 |
| 1 | 洞窟 / 氷原 | 100 | 400 | 300 | 1.2 時間 |
| 2 | 峡谷 / 沼 | 400 | 1000 | 600 | 2.5 時間 |
| 3 | 火山 / 毒沼 | 1000 | 2200 | 1200 | 5 時間 |
| 4 | 湖 / 秘境 | 2200 | 4600 | 2400 | 10 時間 |
| 5 | 氷山 / 冥界 | 4600 | 9400 | 4800 | 20 時間 |
| 6 | 天界 / 遺跡 | 9400 | 19000 | 9600 | 40 時間 |

`cap` は倒すのに要するターン数の式 (`97.3 × フィールドhp / 適正技atk`) に出てこないので、
**戦闘の重さを変えずに滞在時間だけを動かせる**つまみになっている。

`exp` は「1 撃破で 敵Lv +1・自Lv +1」になる値、つまり `個体exp × exp / 2 == 1` から決めている。
`個体exp` の期待値はレア度の抽選確率で決まるので (レア3 が 7% と効く)、
フィールドごとに計算して置いてある。ずれると滞在中に自Lvが敵Lvから離れていくので、
不変条件テストが ±15% で見張っている。

### 通常進行の外にあるフィールド

`/go` の行き先にならない 3 つ。`field-requirements.json` に載っていないもの、が定義。

| フィールド | 到達手段 |
|---|---|
| 地獄 | 魔鏡の「吸い込み」のみ。経験値/ターンを最上位より意図的に悪くしてあり、居座る価値が無い |
| 竜洞 | なし (未完成のまま残っているデータ) |
| 永遠悪夢 | 管理者コマンドのみ (イベント用) |

地獄は行き先から外しているが、**他フィールドの移動元としては残してある**。
これが無いと吸い込まれたプレイヤーが二度と出られない。

`Nightmare` / `Protomare` は旧実装のマクロ検知で出していたペナルティエネミーで、
法外な個体倍率 (hp 104 / atk 7.4) はその出自によるもの。個体倍率の上限は課さないが、
下位フィールドで召喚されたときに経験値の天井を飛び越えないよう exp だけは通常の帯に収めてある。

### 狙っている値

- 適正スキル 2 回で個体HP倍率 1.0 の敵を倒せる (参入直後がいちばん厳しく、上限に近づくと楽になる)
- 1 体倒すあいだに失う体力は最大体力の 12%〜45%、階層順に単調増加
- 経験値/ターンの天井は 186 (草原) 〜 35530 (天界・遺跡) で単調増加
- スキル威力は習得レベル順に単調非減少 (旧データでは lv400 常闇 112 と lv1000 シグルイ 444 が突出し、
  lv500〜650 と lv1100〜1600 の技が習得時点で下位互換になっていた)。
  習得レベルも階層に合わせて広げてあり、各階層の参入Lv ちょうどに攻撃技が増える
- 個体倍率はレア度ごとの上限に収める。スケールはフィールド側が持つ。
  下限は課さない (「踊る金貨」はギル用の敵なので exp 0.5 のままで正しい)
