 ...
 *
 * ⚠ これは移植時に一度だけ使ったワンショットで、いまは凍結してある。実行しないこと。
 *
 * 移植のあいだは data/*.json をこのツールで生成し、legacy と一致することを CI で
 * 見張っていた。バランス調整で data/ 側を意図的に legacy から動かしたので、
 * いま走らせると調整がまるごと巻き戻る。**以後 data/*.json が正**で、
 * legacy/mmo/*.js は移植元の記録として読むだけ。
 *
 * 残してあるのは「移植時にどう変換したか」の記録としての価値があるため
 * (既定値の埋め方、pic の URL 前置詞の剥がし方、Infinity → null の扱いなど)。
 *
 * 敵 77 体・技 90 個・状態異常 37 種を手で書き写すと必ず転記ミスが出るため、
 * 元の JavaScript モジュールをそのまま require() して JSON に落としていた。
 *
